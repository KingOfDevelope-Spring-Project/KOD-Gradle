...
public class ProductDAO {
	...
	private static final String SELECTALL_QUARTERLY_STATISTICS=
			...
			+ "    QUARTERLY_SALES.QUARTERLY_REVENUE "
			+ "FROM ( "
			+ "    SELECT  "
	...
	private static final String SELECTALL_MONTHLY_REVENUE_FOR_1YEAR=
			...
			+ "    SUM((pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE) - (pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE * IFNULL(c.COUPON_DISCOUNT_RATE, 0) / 100)) AS '월매출' "
			+ "FROM  "
			+ "    ORDERLIST ol "
			+ "INNER JOIN  "
	...
	private static final String SELECTALL_DAILY_REVENUE_FOR_30DAYS=
			...
			+ "    DAY(ol.ORDERLIST_DATE) AS 'MONTH', "
			+ "    SUM((pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE) - (pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE * IFNULL(c.COUPON_DISCOUNT_RATE, 0) / 100)) AS 'DAILY_REVENUE' "
			+ "FROM  "
			+ "    ORDERLIST ol "
			+ "INNER JOIN  "
			+ "    ORDERCONTENT pc ON ol.ORDERLIST_ID = pc.ORDERLIST_ID "
