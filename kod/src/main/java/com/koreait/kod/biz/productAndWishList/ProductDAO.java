...
public class ProductDAO {
	...
	private static final String SELECTALL_DAILY_REVENUE_FOR_30DAYS=
			...
			+ "    YEAR(ol.ORDERLIST_DATE) AS 'YEAR', "
			+ "    MONTH(ol.ORDERLIST_DATE) AS 'MONTH', "
			+ "    DAY(ol.ORDERLIST_DATE) AS 'DAY', "
			+ "    FLOOR(SUM((pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE) - (pc.ORDERCONTENT_CNT * p.PRODUCT_PRICE * IFNULL(c.COUPON_DISCOUNT_RATE, 0) / 100))) AS 'DAYLY_REVENUE' "
			+ "FROM  "
			+ "    ORDERLIST ol "
			+ "INNER JOIN  "
			+ "    ORDERCONTENT pc ON ol.ORDERLIST_ID = pc.ORDERLIST_ID "
