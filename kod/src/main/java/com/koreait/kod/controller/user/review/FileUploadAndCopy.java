...
public class FileUploadAndCopy {

	public List<String> uploadAndCopy(String uploadFilePath, List<MultipartFile> uploadReviewImages) throws IOException {
	    // UUID 목록을 저장할 리스트 생성
	    ...
	    for (MultipartFile uploadReviewImage : uploadReviewImages) {
	        ...
	        if (originalFilename != null && !originalFilename.isEmpty()) {
	            ...
	            try {
	                ...
	                // 이미지인 경우에만 썸네일 생성
	                if (uploadReviewImage.getContentType() != null && uploadReviewImage.getContentType().startsWith("image")) {
	                    // 썸네일 파일 생성
